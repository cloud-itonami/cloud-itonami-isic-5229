(ns freightforwarding.operation
  "OperationActor -- one freight-forwarding/customs-brokerage
  coordination operation = one supervised actor run, expressed as a
  langgraph-clj StateGraph. The advisor (FreightForwardingAdvisor) is
  sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Freight Forwarding Governor (:govern) and the rollout
  ...
  the decision to a human operator (freight-forwarding operator /
  customs broker). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:flag-compliance-concern` ALWAYS
...
(defn- op->store-effect
  ...
  [op]
  (case op
    :log-shipment-record          :shipment/log
    :schedule-logistics-operation :schedule/propose
    :coordinate-carrier-booking   :carrier/coordinate
    :flag-compliance-concern      :concern/record))
...
(defn build
  ...
  `freightforwarding.store/Store`).
  opts:
    :advisor      -- a `freightforwarding.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FreightForwardingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Freight Forwarding Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
